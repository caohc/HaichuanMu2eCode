...
namespace mu2e{
  ...
  void STMResamplingProducer::produce(art::Event& event)
  {
    ...
    for (const StepPointMC& step : StepPointMCs)
   {
       	 flag=0;
	 count=0;
         if ( step.volumeId() == 116){_outputStepPointMCs->emplace_back(step); flag=1;}
 	 ...
 	 if (flag==1)
             ...
             if ( step2.volumeId() == 10)
	    {
                 ...
                 if (relation.relationship() == MCRelationship::same)
		 {
		   ...
		   break;
            ...
            if(count==1) 
	    {
	      _outputStepPointMCs->emplace_back(vd10_particle[0]);
		...
		_outputStepPointMCs->emplace_back(vd10_particle[0]);
	      ...
	      {
	        _outputStepPointMCs->emplace_back(vd10_particle[1]);
